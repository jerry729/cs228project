...
def convert_examples_to_features(examples, tokenizer, args,stage=None):
    ...
    for example_index, example in enumerate(tqdm(examples,total=len(examples))):
        ...
        ast = cov_ast.get_ast_vector(example.source)     
        
        #reindex
        ast_to_ast = []
        ast_to_code = []
        index = 0
        for idx, x in enumerate(ast):
            if idx == 0:
                continue
            if ori2cur_pos[index][1] == (idx-1):
                index += 1
            if x != 0:
                ast_to_code.append(ori2cur_pos[index])
                ast_to_ast.append([x])
        ...
        if example_index < 5:
            if stage=='train':
                ...
                logger.info("ast_to_code: {}".format(' '.join(map(str, ast_to_code))))
                logger.info("ast_to_ast: {}".format(' '.join(map(str, ast_to_ast)))) 
...
class TextDataset(Dataset):
    ...
    def __getitem__(self, item):
        ...
        for idx,nodes in enumerate(self.examples[item].ast_to_ast):
            for a in nodes:
